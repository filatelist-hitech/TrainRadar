...
Любой пользователь может открыть M1 read-only map/schedule для коридора Москва-Павелецкая →
Узуново. Экран не требует аккаунта, не запрашивает location и не показывает реальное положение
поезда. Расписание приходит online через Яндекс.Расписания API; при недоступности сети оно явно
unavailable, а не offline/stale. Каждый экран с такими данными показывает attribution Яндекса.
